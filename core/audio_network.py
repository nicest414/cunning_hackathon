...
- 送信: ggwave.encode() / raw__encode() で超音波データを生成し、PyAudio で再生する
- 受信: マイク入力を ggwave.decode() / raw__decode() へ渡して JSON を復元する
...
_ULTRASOUND_FASTEST_BASE_PEAK_HZ = 15773.4
_DEFAULT_TARGET_TX_HZ = 17000.0
...
# stream.read の連続失敗時に高速ループしないための制御値
        ...
        try:
            while self._running:
                try:
                    ...
                    # 例外が恒常的に発生すると高速ループになってしまうため、
                    # 短いスリープと連続失敗回数による終了制御を行う。
